    ...
    def split(self, test_size=0.3, seed=0):
        ...
        if hasattr(self,"wl"):
            ...
            for i in self.wl_list:
                traind[i], val_testd[i] = train_test_split(all[i], test_size=test_size, random_state=seed)
                vald[i], testd[i] = train_test_split(val_testd[i], test_size=0.5, random_state=seed)
            train = []
            val = []
            test = []
            for i in self.wl_list:
