...
class SwiGLU(nn.Module):  # custom
    def __init__(self, in_features, hidden_features, out_features, dtype=torch.float32):
        ...
        self.w2 = nn.Linear(in_features, hidden_features, dtype=dtype)
        self.w3 = nn.Linear(hidden_features, out_features, dtype=dtype)
        self.act = nn.SiLU()
...
class TorchMD_ET(nn.Module):
    ...
    def __init__(
        ...
        # Add learnable scaling parameters for scalar and vector residuals
        self.layer_scales_x = nn.ParameterList([nn.Parameter(torch.ones(hidden_channels) * 1e-4) for _ in range(num_layers)])
        self.layer_scales_vec = nn.ParameterList([nn.Parameter(torch.ones(hidden_channels) * 1e-4) for _ in range(num_layers)])
        ...
        self.out_norm = nn.RMSNorm(hidden_channels, dtype=dtype)
    ...
    def forward(
        ...
        edge_norms = torch.clamp(torch.norm(edge_vec[mask], dim=1), min=1e-8)
        edge_vec[mask] = edge_vec[mask] / edge_norms.unsqueeze(1)
        ...
        for i, attn in enumerate(self.attention_layers):
            ...
            x = x + dx * self.layer_scales_x[i]
            vec = vec + dvec * self.layer_scales_vec[i].unsqueeze(0).unsqueeze(0)
        x = self.out_norm(x)
        # vec = torch.zeros(x.size(0), 3, x.size(1), device=x.device, dtype=x.dtype)
...
class EquivariantMultiHeadAttention(nn.Module):
    ...
    def __init__(
        ...
        if distance_influence in ["values", "both"]:
            ...
            self.dv_proj = SwiGLU(num_rbf, hidden_channels * 3, hidden_channels * 3, dtype=dtype)
    ...
    def message(
        ...
        if dk is None:
            ...
            attn = (q_i * k_j * dk).sum(dim=-1) / math.sqrt(self.head_dim)
